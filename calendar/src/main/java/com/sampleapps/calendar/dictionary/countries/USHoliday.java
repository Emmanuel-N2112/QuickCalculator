...
public class USHoliday extends HolidayDictionary {
    ...
    public List<CHoliday> getObservedPublicHolidays() {
        ...
        populateStaticNationalHolidays().forEach(cHoliday -> {
            CDay observedDay = CDay.createDay(roundDate(cHoliday.getDay().getDate()), getLocale());
    ...
    public List<CHoliday> populateStaticNationalHolidays() {
        ...
        holidays.add(addHoliday(HolidayNames.VALENTINE, HolidayTypes.NATIONAL, USHolidayDate.getValentineDay(getYear())));
        holidays.add(addHoliday(HolidayNames.GOOD_FRIDAY, HolidayTypes.NATIONAL, USHolidayDate.getGoodFriday(getYear())));
        holidays.add(addHoliday(HolidayNames.EASTER, HolidayTypes.NATIONAL, USHolidayDate.getEasterDay(getYear())));
        holidays.add(addHoliday(HolidayNames.EASTER_MONDAY, HolidayTypes.NATIONAL, USHolidayDate.getEasterMonday(getYear())));
        ...
        holidays.add(addHoliday(HolidayNames.NEW_YEAR_EVE, HolidayTypes.NATIONAL, USHolidayDate.getNewYearsEve(getYear())));
    ...
    public List<CHoliday> populateDynamicNationalHolidays() {
        ...
        holidays.add(addHoliday(HolidayNames.MOTHER_DAY, HolidayTypes.OBSERVANCE, getDynamicHoliday(getYear(), Month.MAY, 2, DayOfWeek.MONDAY)));

        // Father's Day - 3rd Sunday in June
        holidays.add(addHoliday(HolidayNames.FATHER_DAY, HolidayTypes.OBSERVANCE, getDynamicHoliday(getYear(), Month.JUNE, 3, DayOfWeek.SUNDAY)));

        // Labor Day - 1st Monday in September
        holidays.add(addHoliday(HolidayNames.LABOUR, HolidayTypes.NATIONAL, getDynamicHoliday(getYear(), Month.SEPTEMBER, 1, DayOfWeek.MONDAY)));
